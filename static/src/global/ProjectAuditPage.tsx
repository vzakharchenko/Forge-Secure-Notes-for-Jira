...
export default function ProjectAuditPage() {
  ...
  const fetchNotes = async (offset: number) => {
    ...
    try {
      ...
      showFlag({
        id: "loadProjectAudit",
        title: "Failed to load project audit",
        description: error.message ?? "Failed to load project audit",
        type: "error",
        appearance: "error",
        isAutoDismiss: true,
      });
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const filename = `project-${projectKey}-${new Date().toISOString().split("T")[0]}.csv`;
    ...
    return (
      <Box padding="space.400" style={{ textAlign: "center" }}>
        <Stack space="space.400" alignInline="center">
          <Spinner size="large" />
          <Text>Loading project audit...</Text>
        </Stack>
      </Box>
    );
  }
    ...
    <Box padding="space.400">
      <Stack space="space.400">
        ...
        {notes.length === 0 ? (
          ...
          >
            <Text>No audit data found for this project</Text>
          </Box>
        ) : (
          <>
            <Table>
