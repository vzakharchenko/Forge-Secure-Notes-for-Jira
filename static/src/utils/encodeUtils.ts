...
export async function calculateHash(data: string, salt: string, iterations = 27000): Promise<string> {
    return bufferToHex(await hashFunction(data, salt, iterations));
}
export async function calculateHashBase64(data: string, salt: string, iterations = 27000): Promise<string> {
...
export async function decryptMessage(payload: { encrypted: string; iv: string; salt: string }, password: string) {
